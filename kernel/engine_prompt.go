...
func vergexHoldRules() string {
	return "- Hold for meaningful moves, do not churn: hold new positions for at least 90 minutes; do not close a small profit inside the +3% win-side noise band before ~3 hours, but cut losses promptly — never wait to bank a loss that is already cheaper than the position's own stop-loss; after closing a symbol wait 4 hours before re-entry; open at most 1-2 new positions per hour. Small in-and-out trades bled this account to death on fees.\n" +
		"- Fees are the main edge killer: a round trip costs ~0.1% of notional. Size stops and targets so the realistic target is well beyond fees.\n" +
		"- Set stops and targets from each coin's own 15m ATR14 (Decision Data Priority #4): stop-loss at 1.5× the coin's 15m ATR14 beyond entry, take-profit at 2× that stop distance (R/R ≈ 1.33). Never use a fixed percentage — scale to the coin's actual volatility. Use the cost/liquidation heatmap only as relative stress/crowding context, never as a stop/target level. Do not exit on small green or small red.\n" +
		"- Profit-taking: when a position is ≥1.5×ATR in profit and momentum stalls (price no longer extending, volume fading), lean toward closing and banking the gain rather than holding to the 2×ATR target — the stop-loss auto-trails 1×ATR behind peak as the downside backstop.\n" +
...
func writeVergexHardConstraints(sb *strings.Builder, accountEquity float64, riskControl store.RiskControlConfig, tradeFiPositionValueRatio float64, zh bool) {
	...
	if zh {
		...
		sb.WriteString(fmt.Sprintf("- Backend caps size so a stop-out risks ≤%.0f%% of equity; a wider stop yields a smaller position.\n", riskControl.RiskPerTradePct))
